...
parser.add_argument('--logging_dir', default ='experiments/group1', help='Directory to save experiment result')
...
parser.add_argument('--synthetic_eval_dir', default ='../data/eval/stylegan_256,../data/eval/progan_256', help='Directory of synthetic images')
...
def build_model(learning_rate=1e-5):
    ...
    model.compile(loss='binary_crossentropy', optimizer=tf.keras.optimizers.Adam(learning_rate=1e-5), metrics=['accuracy', tf.keras.metrics.AUC(), tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
...
if __name__ == '__main__':
    ...
    print('Loading data ...........')
    X_train, Y_train = util.combine_dataset(real_train_img, synthetic_train_img)
    X_eval, Y_eval = util.combine_dataset(real_eval_img, synthetic_eval_img)
